...
from pybroker import Proxy
from pybroker.errors import ProxyConnError, ProxyTimeoutError, ResolveError
from pybroker.negotiators import HttpsNgtr
from pybroker.utils import log as logger
...
def test_log(log):
    ...
    with log(logger.name, level='DEBUG') as cm:
        ...
        assert p._runtimes == []
        assert cm.output == [
            'DEBUG:pybroker:127.0.0.1:80 [INFO]: MSG; Runtime: 0.00',
            'DEBUG:pybroker:127.0.0.1:80 [HTTP]: MSG; Runtime: 0.00',
        ]
